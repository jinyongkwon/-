...
function hitText() { // 몬스터와 캐릭터가 한 행동을 text로 뛰워줌.
    ...
    if (isSkil == true) { // 스킬 공격
        if (isUserAttack == true) {
            $("#textbox").prepend(
                `${User.name}이(가) HP${User.Skil.usehp}을 소모해 ${rand()}번 스킬 ${User.Skil.name}을(를) 사용하였습니다.\n`
            );
            $("#textbox").prepend(
...
function attack() { // 몬스터와 캐릭터를 공격시킴
    ...
    if (userHp() != 0 && monsterHp() != 0) { // user의 피나 monster의 피가 0이 아닐경우에만 실행
        ...
        $("#monster-image").animate({
            ...
            isUserAttack = false;
            $("#monster-image").attr('src', '/image/monster.png');
